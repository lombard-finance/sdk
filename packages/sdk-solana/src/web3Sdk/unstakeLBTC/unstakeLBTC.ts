...
/**
 * @deprecated Use `redeemForBtc` (see `redeemToken/redeemForBtc`) with
 * `tokenMint` set to the LBTC mint (`getConfig(env).lbtcTokenMint`) for LBTC →
 * BTC via Asset Router. This type is tied to the legacy LBTC-program direct
 * `redeem` path and may be removed in a future major version.
 */
 ...
 *
 * @deprecated Use `redeemForBtc` with `tokenMint` set to
 * `getConfig(env).lbtcTokenMint`. This function calls the legacy LBTC program
 * `redeem` instruction directly; prefer the Asset Router flow. May be removed
