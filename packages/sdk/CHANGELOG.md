...
**A millisecond `expiry` was accepted, and set a permit deadline that never lapses.**

`5.4.0` rejects an `expiry` that is not a whole number of seconds, and one that is not in the future. `Date.now()` passed unconverted is neither: it is a positive safe integer, and it is in the future. It cleared both checks, and the permit deadline landed roughly 56,000 years out.
