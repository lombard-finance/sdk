...
describe('useBtcDepositStarknet', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('returns idle state when no provider', () => {
    const { result } = renderHook(() => useBtcDepositStarknet(undefined));
  ...
  it('calls WalletAccount.connect on mount when provider given', async () => {
    ...
    renderHook(() => useBtcDepositStarknet(fakeProvider));
  ...
  it('falls back to wrapped account when WalletAccount.connect fails', async () => {
    ...
    renderHook(() =>
      useBtcDepositStarknet(fakeProvider, undefined, undefined, 'braavos'),
    );
  ...
  it('passes partnerId to config', async () => {
    ...
    renderHook(() => useBtcDepositStarknet(fakeProvider, 'my-partner-id'));
  ...
  it('delegates stake call to useBtcDeposit', async () => {
    ...
    const { result } = renderHook(() =>
      useBtcDepositStarknet({ id: 'braavos' }),
    );
