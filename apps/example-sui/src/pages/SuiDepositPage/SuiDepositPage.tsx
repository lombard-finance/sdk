 ...
 */
export function SuiDepositPage({ env }: SuiStakePageProps) {
  ...
  const {
    ...
    progress,
    reset,
  } = useBtcDepositSui(suiWallet, suiWalletAccount, partnerId, env);

  const handleStartStaking = async (formData: DepositFormData) => {
    setIsStaking(true);
    try {
